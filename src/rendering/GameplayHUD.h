...
public:
	...
	void SetVisible(Element element, bool isVisible);
	...
	// at all) if `controls` or `isHoldEnabled` differ from what it was last
	// built with -- a no-op most frames, so it is cheap to call every Update().
	void RefreshControlsLegend(const ControlSettings& controls, bool isHoldEnabled);

	[[nodiscard]] bool HoldVisible() const;
	[[nodiscard]] sf::FloatRect HoldPreviewArea() const;

	[[nodiscard]] bool NextVisible() const;
	[[nodiscard]] sf::FloatRect NextPreviewArea() const;

private:
	// One stat inside a panel (LEVEL, TIME, SCORE, LINES): a small caption over
	// a bigger value, both centered on the row -- the same shape the old single
	...
	void DrawPanelFrame(sf::RenderTarget& target, NineSliceFrame& frame, float pulse, int pulseRank) const;
	[[nodiscard]] PromptMode CurrentPromptMode() const;
	void BuildControlsLegend(const ControlSettings& controls, bool isHoldEnabled, PromptMode mode);
	...
	mutable NineSliceFrame leftFrame;
	sf::Text holdCaption;
	sf::FloatRect holdBoxBounds;

	// empty outline drawn behind the held piece's preview sprite (BoardRenderer::RenderHoldPreview);
	// stays empty with nothing held
	sf::RectangleShape holdPlaceholder; 
	sf::RectangleShape leftDivider;
	StatRow levelRow;
	StatRow timeRow;
	bool isHoldVisible = true;
	float leftPanelPulse = 0.f;   // 0..1, decaying -- set by OnLevelUp()

	// Right panel: NEXT queue + SCORE + LINES.
	sf::RectangleShape rightFill;
	mutable NineSliceFrame rightFrame; // see leftFrame
	sf::Text nextCaption;
	...
	NineSliceFrame controlsFrame;
	std::vector<ControlEntry> controlsEntries;
	bool areControlsShown = true;
