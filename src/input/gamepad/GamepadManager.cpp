...
namespace
{
	...
	// How far past center counts as "the player is actively using the gamepad".
...
void GamepadManager::Update()
{
	...
	const unsigned int hardDropButton =
		(layout == Layout::PlayStation) ? PlayStationHardDropButton : XboxHardDropButton;
	const bool isHardDropButtonDown = IsButtonPressed(hardDropButton);

	isHardDropEdge = isHardDropButtonDown && !wasHardDropDown;
	wasHardDropDown = isHardDropButtonDown;

	const bool isRightBumperButtonDown = IsButtonPressed(RightBumperButton);
	isRotateClockwiseEdge = isRightBumperButtonDown && !wasRightBumperDown;
	wasRightBumperDown = isRightBumperButtonDown;

	const bool isLeftBumperButtonDown = IsButtonPressed(LeftBumperButton);
	isRotateCounterClockwiseEdge = isLeftBumperButtonDown && !wasLeftBumperDown;
	wasLeftBumperDown = isLeftBumperButtonDown;

	const bool isHoldButtonDown = IsButtonPressed(HoldButton);
	isHoldEdge = isHoldButtonDown && !wasHoldDown;
	wasHoldDown = isHoldButtonDown;
}
...
GamepadManager::NavigationAction GamepadManager::GetNavigationAction(const sf::Event& event) const
{
	...
	{
		// A soft rumble tick only -- no lightbar flash, so the resting color
		// (the focused menu item's hue) stays put as the selection moves.
		const HapticSettings::Rumble& rumble =
			hapticSettings != nullptr ? hapticSettings->menuNavigation : FallbackMenuRumble;

		Haptics::TriggerPulse(*haptics, rumble);
	}
...
GamepadManager::NavigationAction GamepadManager::ComputeNavigationAction(const sf::Event& event) const
{
	if (const auto* moved = event.getIf<sf::Event::JoystickMoved>())
	{
		...
		{
			if (position < -DirectionThreshold)
	...
	const unsigned int confirmButton =
		(layout == Layout::PlayStation) ? PlayStationConfirm : XboxConfirm;
	const unsigned int backButton =
		(layout == Layout::PlayStation) ? PlayStationBack : XboxBack;
