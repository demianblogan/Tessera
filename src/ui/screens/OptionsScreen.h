...
#include "../panels/OptionsCategoryPanel.h"
...
private:
	...
	[[nodiscard]] UI::MenuButtonColumn& GetSubColumn(std::size_t categoryRow);
	[[nodiscard]] UI::MenuButtonColumn& GetActiveSubColumn();

	// Marks the currently active language's own entry so it stays identifiable
	// after leaving and returning to this sub-page.
	[[nodiscard]] sf::String GetLanguageButtonLabel(Language language, std::string_view key) const;
	...
	// The haptics.json key and on-screen fallback color for whatever is focused.
	[[nodiscard]] std::pair<std::string_view, sf::Color> GetCurrentLightbar() const;
	...
	float pageFraction = 0.f; // 0..1 progress through a slide
