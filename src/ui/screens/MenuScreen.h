...
// Default header-return pose: the header slot itself (no morph target set).
// constexpr at namespace scope already has internal linkage, so each
// translation unit that includes this header gets its own copy.
constexpr sf::Vector2f DefaultHeaderReturnCenter{ 960.f, 120.f };
constexpr float DefaultHeaderReturnHeight = 110.f;
...
public:
	...
	// sent StartExit() and then kept updating / rendering until IsExitFinished()
	...
	[[nodiscard]] virtual bool IsExitFinished() const;
	...
	[[nodiscard]] virtual sf::Vector2f GetHeaderReturnCenter() const;
	[[nodiscard]] virtual float GetHeaderReturnHeight() const;

	// The lightbar color to hold while this screen is active; std::nullopt
	// leaves the lightbar untouched (e.g. during an intro animation).
	[[nodiscard]] virtual std::optional<sf::Color> GetLightbarColor() const;
