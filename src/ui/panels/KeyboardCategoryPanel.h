...
private:
	void SyncRows();
	void BeginCapture(std::size_t index, bool isFromKeyboard);
	void EndCapture();
	void TryAssign(std::size_t index, sf::Keyboard::Scancode key);

	[[nodiscard]] static bool IsReserved(sf::Keyboard::Scancode key);
	[[nodiscard]] std::optional<std::size_t> GetClashingRow(std::size_t index, sf::Keyboard::Scancode key) const;
