...
namespace
{
	...
	// No OptionsSfx helper matches these pitches, so they are named locally.
	constexpr float CaptureStartPitch = 1.1f;
	constexpr float InvalidKeyPitch = 0.55f;
	constexpr float ClashKeyPitch = 0.5f;
	constexpr float AcceptKeyPitch = 1.15f;
	constexpr float CaptureCancelPitch = 0.7f;
}
...
{
	fields = {
		&ControlSettings::moveTetrominoLeft,
		&ControlSettings::moveTetrominoRight,
		&ControlSettings::softDropTetromino,
		&ControlSettings::hardDropTetromino,
		&ControlSettings::rotateTetrominoClockwise,
		&ControlSettings::rotateTetrominoCounterClockwise,
		&ControlSettings::holdTetromino };
...
void KeyboardCategoryPanel::BuildRows()
{
	...
	for (std::size_t i = 0; i < ActionCount; ++i)
	{
		...
		auto row = std::make_unique<UI::KeyBindRow>(font, text.GetText(labels[i]), GetKeyName(key));
		rowPtrs[i] = row.get();
...
void KeyboardCategoryPanel::SyncRows()
{
	for (std::size_t i = 0; i < ActionCount; ++i)
	{
		rowPtrs[i]->SetKeyLabel(GetKeyName(working.controls.*fields[i]));
	}
}

bool KeyboardCategoryPanel::IsSettingsEqual(const GameSettings& current, const GameSettings& saved) const
{
	for (const Field field : fields)
	{
		if (current.controls.*field != saved.controls.*field)
		{
			return false;
		}
	}
...
GameSettings KeyboardCategoryPanel::GetDefaultSettings() const
{
...
void KeyboardCategoryPanel::BeginCapture(std::size_t index, bool isFromKeyboard)
{
	...
	captureIgnore = isFromKeyboard
		? std::optional<sf::Keyboard::Scancode>(sf::Keyboard::Scancode::Enter)
		: std::nullopt;

	context.audioPlayer.Play(Assets::SoundID::MenuItemPressed, CaptureStartPitch);
}

void KeyboardCategoryPanel::EndCapture()
{
	if (capturingRow.has_value())
	{
...
bool KeyboardCategoryPanel::IsReserved(sf::Keyboard::Scancode key)
{
	...
	case Scancode::LShift:  case Scancode::RShift:
	case Scancode::LControl: case Scancode::RControl:
	case Scancode::LAlt:    case Scancode::RAlt:
	case Scancode::LSystem: case Scancode::RSystem:
	case Scancode::Menu:
	case Scancode::CapsLock: case Scancode::NumLock: case Scancode::ScrollLock:
...
std::optional<std::size_t> KeyboardCategoryPanel::GetClashingRow(std::size_t index, sf::Keyboard::Scancode key) const
{
	for (std::size_t i = 0; i < ActionCount; ++i)
	{
...
void KeyboardCategoryPanel::TryAssign(std::size_t index, sf::Keyboard::Scancode key)
{
	...
	if (IsReserved(key))
	{
		...
		audio.Play(Assets::SoundID::MenuItemPressed, InvalidKeyPitch);
		EndCapture();
		return;
	}

	if (const std::optional<std::size_t> other = GetClashingRow(index, key); other.has_value())
	{
		rowPtrs[index]->Flash(FlashClash);
		rowPtrs[*other]->Flash(FlashClash);
		audio.Play(Assets::SoundID::MenuItemPressed, ClashKeyPitch);
		EndCapture();
		return;
	}

	working.controls.*fields[index] = key;
	rowPtrs[index]->SetKeyLabel(GetKeyName(key));
	rowPtrs[index]->Flash(FlashAccept);
	audio.Play(Assets::SoundID::MenuItemSelected, AcceptKeyPitch);
	EndCapture();
}
...
bool KeyboardCategoryPanel::HandleEvent(const sf::Event& event)
{
	if (capturingRow.has_value())
	{
		...
		{
			if (captureIgnore.has_value() && released->scancode == *captureIgnore)
			{
				captureIgnore.reset();
			}
			return true;
		...
		{
			if (captureIgnore.has_value() && pressed->scancode == *captureIgnore)
			{
				return true;
			}
			if (pressed->scancode == sf::Keyboard::Scancode::Escape)
			{
				context.audioPlayer.Play(Assets::SoundID::MenuItemSelected, CaptureCancelPitch);
				EndCapture();
				return true;
			}
		...
		{
			if (MenuInput::ResolveAction(event, context.gamepad) == MenuInput::Action::Back)
			{
				context.audioPlayer.Play(Assets::SoundID::MenuItemSelected, CaptureCancelPitch);
				EndCapture();
			}
...
bool KeyboardCategoryPanel::IsStayOpenRequested() const
{
	return capturingRow.has_value() || SettingsCategoryPanel::IsStayOpenRequested();
}
