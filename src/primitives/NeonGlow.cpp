...
{
	return MinPulseIntensity + PulseAmplitude * (std::sin(elapsedTime * PulseSpeed) * 0.5f + 0.5f);
}
...
{
	const sf::Vector2f texelStep
	{
		radius / static_cast<float>(input.getSize().x),
		radius / static_cast<float>(input.getSize().y)
	};
...
void NeonGlow::Draw(sf::RenderTarget& target, sf::FloatRect area, const DrawSource& drawSource, sf::Color tint, bool isPulsing)
{
	...
	const float pulse = isPulsing ? Pulse() : 1.f;
