...
public:
	...
	void Draw(sf::RenderTarget& target, sf::FloatRect area, const DrawSource& drawSource,
		sf::Color tint, bool isPulsing = true);
...
private:
	...
	static constexpr float MinPulseIntensity = 0.78f;   // never fully dims -- just breathes
	static constexpr float PulseAmplitude = 0.22f;      // MinPulseIntensity + PulseAmplitude = full brightness
