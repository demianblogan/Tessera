	...
	for (int type = 0; type < 7; type++)
	{
		for (int rotation = 0; rotation < PieceData::RotationCount; rotation++)
		{
			const PieceData::BlockOffsets& blocks =
				PieceData::Blocks(static_cast<Tetromino::Type>(type), rotation);
			...
			{
				CHECK(offset.x >= 0);
				CHECK(offset.x < TetrominoShapes::MATRIX_SIZE);
				CHECK(offset.y >= 0);
				CHECK(offset.y < TetrominoShapes::MATRIX_SIZE);
			}
		}
	}
}
...
TEST_CASE("Blocks wraps the rotation index into 0..3")
{
	...
	CHECK(CellSet(PieceData::Blocks(Tetromino::Type::L, 4))
		== CellSet(PieceData::Blocks(Tetromino::Type::L, 0)));
	CHECK(CellSet(PieceData::Blocks(Tetromino::Type::L, -1))
		== CellSet(PieceData::Blocks(Tetromino::Type::L, 3)));
}
...
TEST_CASE("the O piece is identical in all four states")
{
	...
	const auto spawn = CellSet(PieceData::Blocks(Tetromino::Type::O, 0));
	for (int rotation = 1; rotation < 4; rotation++)
	{
		CHECK(CellSet(PieceData::Blocks(Tetromino::Type::O, rotation)) == spawn);
	}
}
...
TEST_CASE("S and Z now have four distinct rotation states")
{
	...
	for (const Tetromino::Type type : { Tetromino::Type::S, Tetromino::Type::Z })
	{
		...
		{
			states.insert(CellSet(PieceData::Blocks(type, rotation)));
		}
		CHECK(states.size() == 4);
	}
}
...
{
	PieceData::ResetToDefaults();
	const auto original = CellSet(PieceData::Blocks(Tetromino::Type::T, 0));
	...
	CHECK(CellSet(PieceData::Blocks(Tetromino::Type::T, 0))
		== std::set<std::pair<int, int>>{ { 0, 0 }, { 1, 0 }, { 2, 0 }, { 3, 0 } });

	PieceData::ResetToDefaults();
	CHECK(CellSet(PieceData::Blocks(Tetromino::Type::T, 0)) == original);
}
